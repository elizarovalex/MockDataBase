...
namespace MockDataBase.Tests
{
	...
	public class MockDbTest
	{
		...
		public void ShouldManyResults()
		{
			...
			// Then
			var reader = _target.AsDbConnection().CreateCommand().ExecuteReader();
		...
		public void ShouldManyReaders()
		{
			...
			// Then
			var connection = _target.AsDbConnection();
		...
		public void ShouldExecuteScalar1()
		{
			...
			// Then
			var value = _target.AsDbConnection().CreateCommand().ExecuteScalar();
		...
		public void ShouldExecuteNonQuery()
		{
			...
			// When
			_target.NewNonQuery(7);

			// Then
			var value = _target.AsDbConnection().CreateCommand().ExecuteNonQuery();
		...
		public void ShouldFieldOperationSupported()
		{
			...
			// When
			_target.NewReader(fields).WithFieldTypes(types);
		...
		private void SetupNewReaderRow(out int @int, out string @string, out DateTime dateTime, out Guid guid)
		{
			...
			// When
			_target.NewRow(@int, @string, @dateTime, @guid);
		}
